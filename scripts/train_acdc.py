...
from src.models.unet import PGE_UNet
...
def evaluate_metrics(model, dataloader, device, num_classes=4):
    ...
    Updated to handle list output from UNet++ (deep supervision).
    """
    model.eval()
    tp = [0] * num_classes
    fp = [0] * num_classes
...
# Initialize model
model = PGE_UNet(n_channels=NUM_SLICES, n_classes=NUM_CLASSES, deep_supervision=True).to(DEVICE)
...
epochs_no_improve = 0
model_save_name_dice = "weights/best_model_acdc_dice.pth"
model_save_name_hd95 = "weights/best_model_acdc_hd95.pth"
model_save_name_overall = "weights/best_model_acdc_overall.pth"
...
# Load best model
print("\nLoading best model...")
model.load_state_dict(torch.load("weights/best_model_acdc.pth"))
model.eval()
