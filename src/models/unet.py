"""
PGE-UNet - Implemented from UNet++ architecture
"""
...
class NAE_Block(nn.Module):
    """
    Noise-Aware Encoder block with ePURE noise estimation and adaptive smoothing.
    """
...
class PRD_Block(nn.Module):
    """
    Physics-Regularized Decoder block with Maxwell Solver integration.
    """
...
class PGE_UNet(nn.Module):
    ...
    def __init__(self, n_channels=1, n_classes=4, deep_supervision=True):
        ...
        # --- Encoder (Column j=0) ---
        self.conv0_0 = NAE_Block(n_channels, channels[0])
        self.conv1_0 = NAE_Block(channels[0], channels[1])
        self.conv2_0 = NAE_Block(channels[1], channels[2])
        self.conv3_0 = NAE_Block(channels[2], channels[3])
